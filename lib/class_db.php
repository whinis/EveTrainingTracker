...
class db
{
	...
	public function selectWhere($table,$where,$columns=null,$limit=null,$sort=null,$group=null,$have=null,$join=null){
		...
		if(is_array($table)) {
			reset($table);
			$statement.=" FROM `".$this->prefix.key($table)."` `".current($table)."`";
		}else
		...
		if(isset($join)){
			...
			foreach($join as $jTable) {
				if (count($jTable) > 2) {
					...
					$statement .= " INNER JOIN " . $this->escape($this->prefix . $jTable[0], "`", true);
					$statement .= " ON " . $this->escape($this->prefix . $jTable[0] . "." . $jTable[1], "`", true) . "=" . $this->escape($this->prefix . $jTable[2] . "." . $jTable[3], "`", true);
				}
				else{
					$tableName = substr($jTable[0],0,strpos($jTable[0],"."));
					$statement .= " INNER JOIN " . $this->escape($this->prefix . $tableName , "`", true) . " ON " . $this->escape($this->prefix . $jTable[0], "`", true) . "=" . $this->escape($this->prefix . $jTable[1], "`", true);
				}
			}
		}
				...
				switch($value[0]){
					case "increment":
						if(isset($value[1]))
							...
							$amount=1;
						$statement.=$this->escape($column,"`",true)."=".$this->escape($column,"`",true)."+".$this->escape($amount,"'").",";
						break;
					case "decrement":
						if(isset($value[1]))
							$amount=$value[1];
